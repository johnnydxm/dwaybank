    ...
    if (!user) {
      ...
      if (demoUser && demoUser.password === password) {
        user = {
          ...
          password_hash: await bcrypt.hash(demoUser.password, 12), // For verification
        };
      }
    }
...
app.post('/api/v1/auth/logout', authenticateToken, async (req: any, res) => {
  try {
    // In a full implementation, we would invalidate the token in Redis
...
async function initializeDatabases() {
  ...
  try {
    ...
    console.warn('⚠️ PostgreSQL connection failed:', error instanceof Error ? error.message : 'Unknown error');
